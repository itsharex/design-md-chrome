export function generateDesignMarkdown(context) {
  ...
  const inferenceDetails = formatInferenceDetails(siteProfile);
...
function formatInferenceDetails(siteProfile) {
  if (!siteProfile || !siteProfile.confidence) {
    return "";
  }
  const evidence = (siteProfile.evidence || []).slice(0, 3).join("; ");
  if (!evidence) {
    return `- Audience/surface inference confidence: ${siteProfile.confidence}.`;
  }
  return `- Audience/surface inference confidence: ${siteProfile.confidence} (${evidence}).`;
}
